...
function AddRadiologyAccount(props) {
    ...
    const handleOnSubmit = async () => {
        ...
        try {
            const response = await fetch("https://corecare-server.onrender.com:5000/healthcareproviders/addhealthcareprovider", {
                method: "POST",
                body: formData
            });
            if (response.ok) {
                ...
                toast.current.show({ severity: 'error', summary: 'Error', detail: `Failed Adding Radiology` });
            }
        } catch (error) {
            console.error(error.message);
    ...
    return (
        <>
            <Toast ref={toast} />
            <Container>
                <AddAccountForm label="Add Radiology" onSubmit={handleOnSubmit}>
...
export default AddRadiologyAccount;